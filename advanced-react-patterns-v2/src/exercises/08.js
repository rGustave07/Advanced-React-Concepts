...
class Toggle extends React.Component {
  ...
  //     const action = typeof updates === "function" ? updates(currentState) : updates;
  //     if (this.props.stateReducer) {
  //       return this.props.stateReducer(currentState, action);
  //     } else {
  //       return updates
  //     }
  //   }, callback)
  ...
  internalSetState(updates, callback) {
    super.setState(currentState => {
      return [updates]
        .map(stateUpdate => typeof stateUpdate === 'function' ? stateUpdate(currentState) : stateUpdate)
        .map(stateUpdate => this.props.stateReducer(currentState, stateUpdate) || {})
        .map(stateUpdate => Object.keys(stateUpdate).length ? stateUpdate : null)[0];
    }, callback)
  }
